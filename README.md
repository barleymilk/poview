...
> Status: **Not Ready for Run** (스캐폴딩 전 문서 단계)
...
- 현재 저장소 상태: 설계 문서 확정 단계(앱 스캐폴딩 전)
- 실행 커맨드 활성화 조건: 스캐폴딩 완료 + `package.json` 스크립트 정의 완료
- Windows 권장 런타임 매니저: `volta` (대안: `nvm-windows`)

### Scaffolding 이후 활성화되는 실행 커맨드

```bash
# 현재 단계(Not Ready)에서는 실행되지 않습니다.
# 아래 명령은 스캐폴딩 이후에만 사용합니다.
...
- 온보딩 체크리스트:
  ...
  - 환경변수: 현재 단계에서는 불필요 (`.env.example`는 앱 스캐폴딩 시점에 추가 예정)
